...
interface TheSeamSchemaFormSubmitSplitHarnessFilters
...
export class TheSeamSchemaFormSubmitSplitHarness extends ComponentHarness {
  ...
  private readonly _button = this.locatorFor(
    TheSeamButtonComponentHarness.with({ type: 'submit' }),
  )
  private readonly _triggerButton = this.locatorFor(
    TheSeamButtonComponentHarness.with({ type: 'button' }),
  )
  ...
  static with(
    options: TheSeamSchemaFormSubmitSplitHarnessFilters,
  ): HarnessPredicate<TheSeamSchemaFormSubmitSplitHarness> {
  ...
  public async isRequired(): Promise<boolean> {
    const required = (await (await this._button()).host()).getAttribute(
      'required',
    )
