...
export class SideNavComponent
  implements OnInit, OnDestroy, TheSeamBaseLayoutNav
{
  ...
  set menuItemTooltipConfig(
    value: SideNavItemMenuItemTooltipConfig | undefined | null,
  ) {
  ...
  @ContentChild(BaseLayoutSideBarHeaderDirective, {
    static: true,
    read: TemplateRef,
  })
  ...
  @ContentChild(BaseLayoutSideBarFooterDirective, {
    static: true,
    read: TemplateRef,
  })
  ...
  constructor(
    ...
    @Optional()
    @Inject(THESEAM_SIDE_NAV_CONFIG)
    private readonly _config?: SideNavConfig,
    @Optional()
    @Inject(THESEAM_BASE_LAYOUT_REF)
    private readonly _baseLayoutRef?: TheSeamBaseLayoutRef,
  ) {
    ...
    this.items$ = this._items.asObservable().pipe(
      map((items) =>
        items && config ? items.map((itm) => applyItemConfig(itm, config)) : [],
      ),
      switchMap((items) =>
        items ? this._sideNav.createItemsObservable(items) : [],
      ),
  ...
  ngOnInit() {
    ...
    if (this._sideBarHeaderTpl) {
      this._sideBarHeaderPortal = new TemplatePortal(
        this._sideBarHeaderTpl,
        this._viewContainerRef,
      )
    }

    if (this._sideBarFooterTpl) {
      this._sideBarFooterPortal = new TemplatePortal(
        this._sideBarFooterTpl,
        this._viewContainerRef,
      )
    }
  }
