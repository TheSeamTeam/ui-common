...
export class DataboardListSortButtonComponent {
  ...
  constructor(
    @Inject(THESEAM_DATABOARDLIST_ACCESSOR)
    private readonly _databoardList: DataboardListComponent,
        ...
        return this._databoardList.dataProps
          ...
          .map((c) => {
            const sort = sorts.find(
              (s) => s.boardProp === this.prop && s.dataProp === c.prop,
            )
            return {
              ...c,
              sortDir: sort?.dir,
  ...
  private _canToggleBoard(
    board: DataboardCardDataProp,
    filter: string,
    omitInternalBoards: boolean = true,
  ): boolean {
    return this._boardMatchesFilter(board, filter)
  }

  private _boardMatchesFilter(
    board: DataboardCardDataProp,
    filter: string,
  ): boolean {
    if (filter.length === 0) {
    ...
    return (
      `${board.prop || ''}`.toLowerCase().indexOf(filter) !== -1 ||
      `${board.name || ''}`.toLowerCase().indexOf(filter) !== -1
    )
  }

  _onChange(dataSort: DataboardSort) {
    this._databoardList.sorts$
      .pipe(
        take(1),
        tap((sorts) => {
          const sortDir = this._toggleSortDir(dataSort)

          const existingSorts = sorts.filter(
            (s) => s.boardProp !== this.prop && s.dataProp !== dataSort.prop,
          )
