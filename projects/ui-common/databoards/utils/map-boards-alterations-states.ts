...
import {
  BoardsAlteration,
  BoardsAlterationState,
} from '../models/board-alteration'
...
export function mapBoardsAlterationsStates(
  states: BoardsAlterationState[],
): BoardsAlteration[] {
  ...
  for (const state of states) {
    try {
      switch (state.type) {
        case 'hide-board':
          alterations.push(new HideBoardBoardsAlteration(state.state, true))
          break
        case 'order':
          alterations.push(new OrderBoardsAlteration(state.state, true))
          break
        case 'sort':
          alterations.push(new SortBoardsAlteration(state.state, true))
          break
        default: {
          if (isDevMode()) {
