...
import {
  DataboardBoardData,
  DataboardConfirmMessage,
  DataboardDragPredicate,
  DataboardDynamicConfirmMessage,
} from '../models/databoard-board'
...
export class DataboardBoardComponent {
  ...
  _cardDrop(event: CdkDragDrop<DataboardCard>) {
    ...
    if (
      ...
      const confirmMessage = this._evaluateConfirmMessage(
        this._confirmReorderMessage,
        event.item,
      )
  ...
  private _setBoardConfig(value: DataboardBoardCards) {
    ...
    const evalDisplayEmptyHeader = this._valueHelper.evalSync(
      value.displayEmptyHeader,
      value,
    )
    this._displayEmptyHeader = notNullOrUndefined(evalDisplayEmptyHeader)
      ? evalDisplayEmptyHeader
      : this._displayEmptyHeader
    ...
    this._emptyText =
      this._valueHelper.evalSync(value.emptyText, value) || this._emptyText

    this._footerTpl = value.footerTpl
    this._footerText = this._valueHelper.evalSync(value.footerText, value)
    const evalDisplayEmptyFooter = this._valueHelper.evalSync(
      value.displayEmptyFooter,
      value,
    )
    this._displayEmptyFooter = notNullOrUndefined(evalDisplayEmptyFooter)
      ? evalDisplayEmptyFooter
      : this._displayEmptyFooter

    const evalCollapsible = this._valueHelper.evalSync(value.collapsible, value)
    this._collapsible = notNullOrUndefined(evalCollapsible)
      ? evalCollapsible
      : this._collapsible

    const evalReorderable = this._valueHelper.evalSync(value.reorderable, value)
    this._reorderable = notNullOrUndefined(evalReorderable)
      ? evalReorderable
      : this._reorderable

    const evalDraggable = this._valueHelper.evalSync(value.draggable, value)
    this._draggable = notNullOrUndefined(evalDraggable)
      ? evalDraggable
      : this._draggable

    const evalSelectable = this._valueHelper.evalSync(value.selectable, value)
    this._selectable = notNullOrUndefined(evalSelectable)
      ? evalSelectable
      : this._selectable
    ...
    this._confirmAdd = notNullOrUndefined(evalConfirmAdd)
      ? evalConfirmAdd
      : this._confirmAdd
    this._confirmAddMessage = value.confirmAddMessage

    const evalConfirmReorder = this._valueHelper.evalSync(
      value.confirmReorder,
      value,
    )
    this._confirmReorder = notNullOrUndefined(evalConfirmReorder)
      ? evalConfirmReorder
      : this._confirmReorder
    this._confirmReorderMessage = value.confirmReorderMessage
  }

  private _evaluateConfirmMessage(
    message: DataboardDynamicConfirmMessage | undefined,
    context: any,
  ): DataboardConfirmMessage {
    ...
    if (notNullOrUndefined(message) && notNullOrUndefined(message.alert)) {
      ...
      if (typeof evalAlert === 'string') {
        ...
        if ('message' in message.alert) {
          alertMessage = this._valueHelper.evalSync(
