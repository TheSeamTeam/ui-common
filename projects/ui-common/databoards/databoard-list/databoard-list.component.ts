...
import {
  DataboardHorizontalLayoutBehavior,
  DataboardMenuBarPosition,
  DataboardVerticalLayoutBehavior,
} from '../models/databoard'
import {
  isNullOrUndefined,
  notNullOrUndefined,
...
export class DataboardListComponent
  implements TheSeamDataboardListAccessor, OnDestroy
{
  @Input()
  ...
  private _trackBoard: TrackByFunction<DataboardBoard> = (index, item) =>
    item.prop
   ...
   */
  @Input() horizontalLayoutBehavior: DataboardHorizontalLayoutBehavior =
    'no-wrap'
   ...
   * its own scrollbar.
   */
  @Input() verticalLayoutBehavior: DataboardVerticalLayoutBehavior =
    'board-scroll'
  ...
  @Input()
  get sortType(): DataFilterSortType {
    return this._sortType
  }
  set sortType(value: DataFilterSortType) {
    if (
      notNullOrUndefined(value) &&
      (value === DataFilterSortType.single ||
        value === DataFilterSortType.multi)
    ) {
  ...
  get displayMenuBarTop(): boolean {
    return this.menuBarPosition === 'top'
  }
  get displayMenuBarBottom(): boolean {
    return this.menuBarPosition === 'bottom'
  }

  @ContentChild(DataFilterMenuBarComponent)
  get menuBarComponent(): DataFilterMenuBarComponent | undefined {
  ...
  private _templateBoards = new BehaviorSubject<
    QueryList<DataboardBoardComponent> | undefined
  >(undefined)
  ...
  constructor(
      ...
      .pipe(
        ...
        switchMap((key) => {
          ...
          return from(
            ...
            switchMap(() =>
              this._boardsAlterations.changes$.pipe(
                startWith(undefined),
                tap(() => {
                  this._preferences.setAlterations(
                    key,
                    this._boardsAlterations.get(),
                  )
                }),
              ),
            ),
        ...
        switchMap((prefsKey) => {
          ...
          return this._preferences.preferences(prefsKey).pipe(
            ...
            map((preferences) => {
              ...
              try {
                alterations = mapBoardsAlterationsStates(
    ...
    this.filterStates$ = this._filtersSubject
      .asObservable()
      .pipe(switchMap((filters) => composeDataFilterStates(filters)))

    this.boards$ = combineLatest([
      this._inputBoards.asObservable(),
      this._templateBoards.asObservable(),
    ]).pipe(
    ...
    this.boardsAndCards$ = combineLatest([
      displayBoards$,
      this.cards$,
      this.sorts$,
    ]).pipe(
      map(([boards, cards, sorts]) =>
        this._buildBoardsAndCards(boards, cards, sorts),
      ),
    )
  }
  ...
  private _buildBoardConfig(
    templateBoards: QueryList<DataboardBoardComponent> | undefined,
  ): DataboardBoard[] {
    if (isNullOrUndefined(templateBoards) || templateBoards.length === 0) {
      return []
    }

    return templateBoards.toArray().map(
      (t) =>
        ({
          prop: t.prop,
  ...
  private _buildBoardsAndCards(
    boards: DataboardBoard[] | undefined,
    cards: DataboardCard[] | undefined,
    ...
    return boards.map((board) => {
      ...
      if (
        isNullOrUndefined(board.contentPredicate) ||
        typeof board.contentPredicate !== 'function'
      ) {
        if (isDevMode()) {
          console.warn(
            `Invalid contentPredicate: ${board.contentPredicate}. No cards will be applied to this board.`,
          )
        }
      } else {
        if (notNullOrUndefined(cards) && cards.length > 0) {
          boardCards = cards.filter((card, index) => {
            try {
              ...
              const filterResult = board.contentPredicate?.call(
                undefined,
                card,
                board,
              )
              ...
              if (isDevMode()) {
                console.warn(
                  `An error occured while executing the contentPredicate. The card will not be applied.\n\n contentPredicate: ${board.contentPredicate}\n\n card: ${JSON.stringify(card)}`,
                )
              }
          ...
          boardCards = sortCards(this.sortType, aggregateSorts, boardCards, [
            board,
          ])
        }
      }
  ...
  _cardDrop(event: CdkDragDrop<DataboardBoardCards>) {
    if (event.previousContainer === event.container) {
      if (
        event.container.data.cards === null ||
        event.container.data.cards === undefined
      ) {
        return
      }
      ...
      if (
        event.container.data.cards === null ||
        event.container.data.cards === undefined ||
        event.previousContainer.data.cards === null ||
        event.previousContainer.data.cards === undefined
      ) {
        return
      }
