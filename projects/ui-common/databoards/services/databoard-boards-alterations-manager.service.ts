...
export class DataboardBoardsAlterationsManagerService {
  private readonly _changesSubject =
    new Subject<BoardsAlterationsChangedEvent>()
  ...
  public add(
    ...
    const removed: BoardsAlterationsChangedRecord[] = this.remove(alterations, {
      emitEvent: false,
    })
  ...
  public apply(
    ...
    databoardsList: TheSeamDataboardListAccessor,
  ): void {
      ...
      switch (alt.type) {
        case 'hide-board': {
          const alteration = new HideBoardBoardsAlteration(
            { boardProp: alt.state.boardProp, hidden: false },
            false,
          )
