...
import {
  CURRENT_DATABOARD_LIST_PREFERENCES_VERSION,
  EMPTY_DATABOARD_LIST_PREFERENCES,
  TheSeamDataboardListPreferences,
...
export class DataboardBoardsPreferencesService {
  ...
  constructor(
    ...
    @Optional()
    @Inject(THESEAM_DATABOARDLIST_PREFERENCES_ACCESSOR)
  ...
  ): Observable<TheSeamDataboardListPreferences> {
    ...
    return this._preferencesManager
      ...
      .pipe(
        map((prefs) => {
          ...
          throw Error(
            `Preferences for key '${preferenceKey}' is not a valid databoard preference.`,
          )
        }),
      )
  }
  ...
  private _isValidDataboardListPreferences(
    prefs: TheSeamPreferencesBase,
  ): prefs is TheSeamDataboardListPreferences {
