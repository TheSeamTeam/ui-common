...
export class OrderBoardsAlteration extends BoardsAlteration<OrderBoardsAlterationState> {
  ...
  constructor(state: OrderBoardsAlterationState, persistent: boolean) {
  ...
  public apply(
    ...
    databoardList: TheSeamDataboardListAccessor,
  ): void {
  ...
  private _isBoardOrderRecordValid(boardOrder: BoardOrderRecord): boolean {
    return (
      notNullOrUndefined(boardOrder.boardProp) &&
      notNullOrUndefined(boardOrder.index)
    )
  }
