...
import {
  ...
  TheSeamDatatableDateColumnFilterableConfig,
  ...
  THESEAM_COLUMNS_DATA_FILTER_DATE_SELECT_SEARCH_TYPES,
} from './models'
import { getFormattedDateForComparison } from './utils'

export class SearchDateColumnsDataFilter extends ColumnsDataFilter<
  TheSeamColumnsDataFilterDateSearchFormState,
  TheSeamColumnsDataFilterDateSearchForm,
  TheSeamColumnsDataFilterDateSearchOptions,
  TheSeamDatatableDateColumnFilterableConfig
> {
  ...
  constructor(
    ...
    this.form = new FormGroup({
      searchType:
        new FormControl<TheSeamColumnsDataFilterDateSearchType | null>(
          notNullOrUndefined(initialValue) ? initialValue.searchType : 'eq',
        ),
      searchText: new FormControl<string | null>(
        notNullOrUndefined(initialValue) ? initialValue.searchText : null,
      ),
      fromText: new FormControl<string | null>(
        notNullOrUndefined(initialValue) ? initialValue.fromText : null,
      ),
      toText: new FormControl<string | null>(
        notNullOrUndefined(initialValue) ? initialValue.toText : null,
      ),
    })
  ...
  public dataFilter(
    data: any[],
    filterValue: Partial<TheSeamColumnsDataFilterDateSearchFormState>,
    options: any,
  ): any[] {
    ...
    return this._dateSearchDataFilter(
      this.prop,
      data,
      filterValue.searchText,
      filterValue.fromText,
      filterValue.toText,
  ...
  private _isInvalidDate(dateString: string | null | undefined): boolean {
    return (
      isNullOrUndefinedOrEmpty(dateString) ||
  ...
  private _isInvalidSearchTerm(
    ...
    comparator: TheSeamColumnsDataFilterDateSearchType,
  ): boolean {
    if (
      THESEAM_COLUMNS_DATA_FILTER_DATE_TEXT_SEARCH_TYPES.includes(comparator) &&
  ...
  private _dateSearchDataFilter(
    ...
    const textDate = getFormattedDateForComparison(
      text,
      this.options.dateType,
      true,
    )
    const fromTextDate = getFormattedDateForComparison(
      fromText,
      this.options.dateType,
      true,
    )
    const toTextDate = getFormattedDateForComparison(
      toText,
      this.options.dateType,
      true,
    )
    ...
    return data.filter((item) => {
      const propDate = getFormattedDateForComparison(
        item[prop],
        this.options.dateType,
        useLocalTime,
      )
  ...
  public isDefault(): boolean {
    ...
    if (
      THESEAM_COLUMNS_DATA_FILTER_DATE_SELECT_SEARCH_TYPES.includes(
