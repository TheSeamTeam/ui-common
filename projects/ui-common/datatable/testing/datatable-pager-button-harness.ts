...
export class TheSeamDatatablePagerButtonHarness extends ComponentHarness {
  ...
  static with(
    options: TheSeamDatatablePagerButtonHarnessFilters,
  ): HarnessPredicate<TheSeamDatatablePagerButtonHarness> {
    return new HarnessPredicate(
      TheSeamDatatablePagerButtonHarness,
      options,
    ).addOption('page number', options.pageNumber, (harness, index) =>
      HarnessPredicate.stringMatches(harness.getLabel(), `page ${index}`),
    )
  }
