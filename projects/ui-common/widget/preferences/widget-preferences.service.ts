...
import {
  CURRENT_WIDGET_PREFERENCES_VERSION,
  EMPTY_WIDGET_PREFERENCES,
  TheSeamWidgetPreferences,
} from './widget-preferences.models'
...
export class WidgetPreferencesService {
  private _pendingPatches: {
    [preferencesKey: string]: TheSeamWidgetPreferencesPatch[]
  } = {}

  constructor(
    private readonly _preferencesManager: TheSeamPreferencesManagerService,
    @Optional()
    @Inject(THESEAM_WIDGET_PREFERENCES_ACCESSOR)
  ...
  ): Observable<TheSeamWidgetPreferences> {
    ...
    return this._preferencesManager
      ...
      .pipe(
        map((prefs) => {
          ...
          throw Error(
            `Preferences for key '${preferenceKey}' is not a valid widget preferences.`,
          )
        }),
      )
  }
  ...
  public patchPreferences(
    preferenceKey: string,
    preferences: TheSeamWidgetPreferencesPatch,
  ): void {
    if (!this._prefsAccessor) {
      return
    }
    ...
    this.preferences(preferenceKey)
      .pipe(
        ...
        map((prefs) => {
          ...
          this._pendingPatches[preferenceKey] = []
          const newPrefs: TheSeamWidgetPreferences = pendingPatches.reduce(
            (acc, patch) => {
              return { ...acc, ...patch }
            },
            prefs,
          ) as TheSeamWidgetPreferences // Typescript isn't recognizing that 'version' is included in the initial value.

          if (!this._isValidPreferences(newPrefs)) {
            throw Error(
              `Attempted to patch preferences for key '${preferenceKey}' with invalid preferences.`,
            )
          }
  ...
  ): prefs is TheSeamWidgetPreferences {
