...
export class TableCellTypeDecimalComponent implements OnDestroy {
  ...
  constructor(
    ...
    _tableData?: TableCellData<'decimal', TableCellTypeConfigDecimal>,
  ) {
    ...
    if (tableData) {
      tableData.changed.pipe(takeUntil(this._ngUnsubscribe)).subscribe((v) => {
        if (Object.prototype.hasOwnProperty.call(v.changes, 'value')) {
          this.value = this._formatDecimal(
  ...
  private _formatDecimal(
    currentValue?: any,
    tableData?:
      | TableCellData<'decimal', TableCellTypeConfigDecimal>
      | undefined,
  ): string {
    ...
    const formatDecimal = notNullOrUndefined(config?.formatNumber)
      ? this._parseConfigValue(
          coerceBooleanProperty(config?.formatNumber),
          tableData,
        )
      : true
    ...
    const minFractionDigits =
      this._parseConfigValue(config?.minFractionDigits, tableData) || 0
    const maxFractionDigits =
      this._parseConfigValue(config?.maxFractionDigits, tableData) || 3
  ...
  private _parseConfigValue(
    val?: any,
    tableData?: TableCellData<'decimal', TableCellTypeConfigDecimal>,
  ) {
    const contextFn = () =>
