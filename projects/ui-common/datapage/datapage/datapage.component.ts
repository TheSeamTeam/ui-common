...
import {
  DatapageAnimation,
  DatapagePageView,
  DatapagePageViewModel,
  isDatapageAnimationProperty,
  isDatapagePageViewProperty,
} from '../models/datapage'
...
export class DatapageComponent implements AfterContentInit {
  ...
  private _pageView = new BehaviorSubject<DatapagePageView | undefined>(
    DatapagePageView.Boards,
  )
   ...
   */
  @Input() defaultViewMessage =
    'Select an option from the buttons above to view your data.'
  ...
  private _pageAnimation = new BehaviorSubject<DatapageAnimation>(
    DatapageAnimation.None,
  )
  public pageAnimation$ = this._pageAnimation.asObservable()

  @ContentChild(DatapageDatatableTplDirective, {
    read: TemplateRef,
    static: true,
    descendants: false,
  })
  ...
  private _datatableTplQuery = new BehaviorSubject<
    TemplateRef<any> | undefined
  >(undefined)
  public datatableTplQuery$ = this._datatableTplQuery.asObservable()

  @ContentChild(DatapageDataboardTplDirective, {
    read: TemplateRef,
    static: true,
    descendants: false,
  })
  ...
  private _databoardTplQuery = new BehaviorSubject<
    TemplateRef<any> | undefined
  >(undefined)
  public databoardTplQuery$ = this._databoardTplQuery.asObservable()

  @ContentChild(DatapageDefaultTplDirective, {
    read: TemplateRef,
    static: true,
    descendants: false,
  })
  ...
  ngAfterContentInit(): void {
    this.pageViewOptions$ = combineLatest([
      this.datatableTplQuery$,
      this.databoardTplQuery$,
    ]).pipe(
    ...
    this.animateStatus$ = combineLatest([
      this.pageAnimation$,
      this.pageView$,
    ]).pipe(map(([pageAnimation, pageView]) => `${pageView}_${pageAnimation}`))
