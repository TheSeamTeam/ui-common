...
@Injectable()
export class ExportersDataEvaluator
  implements IDynamicValueEvaluator<'exporters-data'>
{
  ...
  public async eval(
    value: IExportersDataValue<IDataExporter[]>,
    context?: any,
  ): Promise<{ exporters: IDataExporter[] }> {
  ...
  private _exporters(exporters: string[]) {
    const dataExporters: IDataExporter[] = ((this._injector.get(
      THESEAM_DATA_EXPORTER,
    ) as any) || []) as IDataExporter[]
    return exporters
