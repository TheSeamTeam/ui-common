...
const manifests: DynamicComponentManifest[] = [
  {
    ...
    path: 'widget-one',
    loadChildren: () =>
      import('./lazy-widget-one/lazy-widget-one.module').then(
        (m) => m.LazyWidgetOneModule,
      ),
  },
]

@NgModule({
  declarations: [ExampleLazyLoadedWidgetsComponent],
  imports: [
    CommonModule,

    // TheSeamDynamicComponentLoaderModule.forModule(manifests[0]),
  ],
  exports: [ExampleLazyLoadedWidgetsComponent],
})
